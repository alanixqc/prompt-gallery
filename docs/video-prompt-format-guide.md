...
Reference spec for writing structured audiovisual video-generation prompts. Covers plain text-to-video-audio (T2VA), image-to-video-audio with a first frame (I2VA), first-and-last-frame (FL2VA), last-frame-only (L2VA), and the separate **Full-Reference Mode** used when multiple reference assets (images/videos/audio) are combined (see second half of this document).
...
This guide explains how rewrite outputs are organized and written in **full-reference mode** — used when a prompt combines multiple reference assets (images, videos, audio) rather than a single first/last-frame anchor.
...
The basic formats for shots, camera movement, speakers, dialogue, and ordinary sound are shared with the Video Prompt Writing Guide (T2VA / I2VA / FL2VA / L2VA) above. This guide focuses on the reference labels, analysis sections, and format differences specific to full-reference mode.
...
The basic format follows the Video Prompt Writing Guide (T2VA / I2VA / FL2VA / L2VA) above:
...
For complete rules and examples covering camera vocabulary, group speech, voice-over, dialogue across cuts, and visible text, see the Video Prompt Writing Guide section above.
...
The definitions of these two sound categories follow the Video Prompt Writing Guide (T2VA / I2VA / FL2VA / L2VA) above.
